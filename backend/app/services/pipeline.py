    ...
    def refresh_aggregates(self, run: CalculationRun, dates: list[date]) -> None:
        ...
        grains = [
            ...
            (AggDailyBranchProduct,
             [F.business_date, F.branch_id, F.product_id, F.branch_code,
              F.product_code, F.division_id, F.district_id, F.branch_category,
              F.side],
             lambda r: dict(business_date=r[0], branch_id=r[1], product_id=r[2],
                            branch_code=r[3], product_code=r[4],
                            division_id=r[5], district_id=r[6],
                            branch_category=r[7], side=r[8]), 9),
